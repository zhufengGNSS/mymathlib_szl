////////////////////////////////////////////////////////////////////////////////
// File: adams_18_steps.c                                                     //
// Routines:                                                                  //
//    Adams_18_Steps                                                          //
//    Adams_Bashforth_18_Steps                                                //
//    Adams_Moulton_17_Steps                                                  //
//    Adams_18_Build_History                                                  //
////////////////////////////////////////////////////////////////////////////////
...
static const double bashforth[] = {401972381695456831.0, -2735437642844079789.0,
   13930159965811142228.0, -51150187791975812900.0, 141500575026572531760.0,
  -304188128232928718008.0, 518600355541383671092.0, -710171024091234303204.0,
   786600875277595877750.0, -706174326992944287370.0, 512538584122114046748.0,
  -298477260353977522892.0, 137563142659866897224.0, -49070094880794267600.0,
   13071639236569712860.0, -2448689255584545196.0, 287848942064256339.0,
  -15980174332775873.0 };

static const double moulton[] = { 15980174332775873.0, 114329243705491117.0,
      -290470969929371220.0, 890337710266029860.0, -2250854333681641520.0,
       4582441343348851896.0, -7532171919277411636.0, 10047287575124288740.0,
      -10910555637627652470.0, 9644799218032932490.0, -6913858539337636636.0,
       3985516155854664396.0, -1821304040326216520.0, 645008976643217360.0,
      -170761422500096220.0, 31816981024600492.0, -3722582669836627.0,
       205804074290625.0 };

static const double divisor = 1.0 / 64023737057280000.0;

#define STEPS sizeof(bashforth)/sizeof(bashforth[0])

double Adams_Bashforth_18_Steps( double y, double h, double f_history[] );

int Adams_Moulton_17_Steps( double (*f)(double, double), double y[], double x, 
             double h, double f_history[], double tolerance, int iterations );

static int hasConverged(double y0, double y1, double epsilon);

////////////////////////////////////////////////////////////////////////////////
// int Adams_18_Steps( double (*f)(double, double), double y[], double x0,    //
...
//     i = 1,...,17, stored in the array f_history[], and y(x0) stored in     //
//     y[0].                                                                  //
//                                                                            //
//  Arguments:                                                                //
//     double *f  Pointer to the function which returns the slope at (x,y) of ////                integral curve of the differential equation y' = f(x,y)     //
...
//                f_history[i] = f( x0-(17-i)*h, y(x0-(17-i)*h) ), i = 0,..., //
//                16.  On output, the updated history list,                   //
//                f_history[i] = f( x0-(18-i)*h, y(x0-(18-i)*h) ), i = 0,..., //
//                16.  On the initial call to this routine, f_history[i],     //
//                i = 0,..., 16, must be initialized by the calling routine.  //
//                Thereafter this function maintains the array.  The array    //
//                f_history[] must be dimensioned at least 18 in the calling  //
//                routine.  If the values y(x0-17*h) ,..., y(x0-h) are given, //
//                the user may call Adams_18_Build_History, given below, to   //
...
int Adams_18_Steps( double (*f)(double, double), double y[], double x0, 
   ...
   *y_bashforth  = Adams_Bashforth_18_Steps( y[0], h, f_history );
   for (i = 0; i < STEPS - 1; i++) f_history[i] = f_history[i+1];

          // Calculate the corrector using the Adams-Moulton formula 
   
   y[1] = *y_bashforth;
   return Adams_Moulton_17_Steps( f, y, x0+h, h, f_history, tolerance,
                                                               iterations );
}
...
//     uses the starting values f(x0-i*h,y(x0-i*h)), i = 0,..., 17, stored in //
...
//                f_history[i] = f( x0-(17-i)*h, y(x0-(17-i)*h) ),            //
//                i = 0,..., 17.  The array f_history[] must be dimensioned   //
//                at least 18 in the calling routine.                         //
//                                                                            //
//  Return Values:                                                            //
...
////////////////////////////////////////////////////////////////////////////////
// int Adams_Moulton_17_Steps( double (*f)(double, double), double y[],       //
...
//     using starting values f(x-i*h,y(x-i*h)), i = 1,...,17, stored in the   //
...
//                f_history[i] = f( x-(17-i)*h, y(x-(17-i)*h) ), i = 0,...,   //
//                16.  The array f_history[] must be dimensioned at least 17  //
...
////////////////////////////////////////////////////////////////////////////////
// void Adams_18_Build_History(double (*f)(double,double), double f_history[],//
...
//     y[i] = y(x + i*h) for i = 0,..., 17 are given.  Then                   //
...
//                f_history[17] = f(x+17*h,y[17).                             //
//                The array f_history must be dimensioned at least 18.        //
