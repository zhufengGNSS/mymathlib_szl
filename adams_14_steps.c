////////////////////////////////////////////////////////////////////////////////
// File: adams_14_steps.c                                                     //
// Routines:                                                                  //
//    Adams_14_Steps                                                          //
//    Adams_Bashforth_14_Steps                                                //
//    Adams_Moulton_13_Steps                                                  //
//    Adams_14_Build_History                                                  //
////////////////////////////////////////////////////////////////////////////////
...
static const double bashforth[] = { 27511554976875.0, -140970750679621.0, 
              537247052515662.0, -1445313351681906.0, 2854429571790805.0, 
            -4246767353305755.0, 4825671323488452.0, -4204551925534524.0, 
             2793869602879077.0, -1393306307155755.0, 505586141196430.0, 
             -126174972681906.0, 19382853593787.0, -1382741929621.0 };

static const double moulton[] = { 1382741929621.0, 8153167962181.0, 
               -15141235084110.0, 33928990133618.0, -61188680131285.0, 
                86180228689563.0, -94393338653892.0, 80101021029180.0, 
               -52177910882661.0, 25620259777835.0, -9181635605134.0,
                 2268078814386.0, -345457086395.0, 24466579093.0 };

static const double divisor = 1.0 / 5230697472000.0;

#define STEPS sizeof(bashforth)/sizeof(bashforth[0])

double Adams_Bashforth_14_Steps( double y, double h, double f_history[] );
int Adams_Moulton_13_Steps( double (*f)(double, double), double y[], double x, 
             double h, double f_history[], double tolerance, int iterations );
static int hasConverged(double y0, double y1, double epsilon);

////////////////////////////////////////////////////////////////////////////////
// int Adams_14_Steps( double (*f)(double, double), double y[], double x0,    //
...
//     i = 1,...,13, stored in the array f_history[], and y(x0) stored in     //
//     y[0].                                                                  //
//                                                                            //
//  Arguments:                                                                //
...
//                f_history[i] = f( x0-(13-i)*h, y(x0-(13-i)*h) ), i = 0,..., //
//                12.  On output, the updated history list,                   //
//                f_history[i] = f( x0-(14-i)*h, y(x0-(14-i)*h) ), i = 0,..., //
//                12.  On the initial call to this routine, f_history[i],     //
//                i = 0,...,12, must be initialized by the calling routine.   //
//                Thereafter this function maintains the array.  The array    //
//                f_history[] must be dimensioned at least 14 in the calling  //
//                routine.  If the values y(x0-13*h) ,..., y(x0-h) are given, //
//                the user may call Adams_14_Build_History, given below, to   //
...
int Adams_14_Steps( double (*f)(double, double), double y[], double x0, 
   ...
   *y_bashforth  = Adams_Bashforth_14_Steps( y[0], h, f_history );
   for (i = 0; i < STEPS - 1; i++) f_history[i] = f_history[i+1];

          // Calculate the corrector using the Adams-Moulton formula 
   
   y[1] = *y_bashforth;
   return Adams_Moulton_13_Steps( f, y, x0+h, h, f_history, tolerance,
                                                               iterations );
}
...
//     uses the starting values f(x0-i*h,y(x0-i*h)), i = 0,..., 13, stored in //
...
//                f_history[i] = f( x0-(13-i)*h, y(x0-(13-i)*h) ),            //
//                i = 0,..., 13.  The array f_history[] must be dimensioned   //
//                at least 14 in the calling routine.                         //
//                                                                            //
//  Return Values:                                                            //
...
////////////////////////////////////////////////////////////////////////////////
// int Adams_Moulton_13_Steps( double (*f)(double, double), double y[],       //
...
//     using starting values f(x-i*h,y(x-i*h)), i = 1,...,13, stored in the   //
...
//                f_history[i] = f( x-(13-i)*h, y(x-(13-i)*h) ), i = 0,...,   //
//                12.  The array f_history[] must be dimensioned at least 13  //
...
////////////////////////////////////////////////////////////////////////////////
// void Adams_14_Build_History(double (*f)(double,double), double f_history[],//
...
//     y[i] = y(x + i*h) for i = 0,..., 13 are given.  Then                   //
...
//                f_history[13] = f(x+13*h,y[13]).                            //
//                The array f_history must be dimensioned at least 14.        //
